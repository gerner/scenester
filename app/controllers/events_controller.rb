...
class EventsController < ApplicationController
  ...
  def index
    ...
    @events = Event.paginate(:conditions => ["start > ? AND ((tags <> ? AND lower(tags) not like ?) OR recommended IS 'true')", now, "Happy Hours", "%music%"], :page => params[:page], :per_page => params[:per_page], :order => 'start ASC', :include => [:venue])
